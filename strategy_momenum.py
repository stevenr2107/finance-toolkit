...
import pandas as pd # verwaltet Tabellen wie excel in python
...
@dataclass # Einstellungs container - alle Parameter der strategie werden hier definiert
# Wie Risikomanagement 
class MomentumConfig:
    """ Momentum Berechnung """
    # Momentum Berechnung 
    momentum_window: int = 126 # 6 Monate zurückschauen
    ...
    # Risiko Management
    stop_loss_pct: float = 0.07 # 7% stop loss - ausstieg bei 7% verlust
    take_profit_pct: float = 0.2 # 20% take profit - ausstieg bei 20% profit
    max_hold_days: int = 60 # Maximal 60 Tage halten
    
    # Sizing
    risk_per_trade: float = 0.02 # max 2% Risiko pro Trade
...
def compute_momentum_indicators (df: pd.DataFrame,
    ...
    df["ret_1m"]  = close.pct_change(21) # wieviel hat die aktie in den letzten 21 Tagen gewonnen/verloren
    df["ret_3m"]  = close.pct_change(63) # 3-monats return
    df["ret_6m"]  = close.pct_change(126) # 6-monats return 
    df["ret_12m"] = close.pct_change(252) # 12 monats -return

    # Jegadeesh-Titman: 12M Return minus letzter Monat 
    df["jt_momentum"] = df["ret_12m"] - df["ret_1m"] # 1993 forschung - letzer monat zeigt übertreibungen
    # man rechnet letzten monat raus, da der oft übertreibt 

    # Gewichteter Momentum Score - ältere perioden bekommen mehr Gewicht - glattes signal
    df["momentum_score"] = (
        df["ret_1m"] * 0.2 +
        df["ret_3m"] * 0.3 +
        df["ret_6m"] * 0.5
...
def compare_strategies(results: dict) -> pd.DataFrame:
    ...
    for name, result in results.items():
        ...
        # CAGR - jährlicher zinseszins return - Gesamtperformance normiert 
        vol = returns.std() * np.sqrt(252) * 100
        sharpe = (returns.mean() / returns.std()) * np.sqrt (252)
        # Return / vol * wurzel(252) - Rendite pro risikoeinheit 

        rolling_max = equity.cummax()
        max_dd = ((equity - rolling_max) / rolling_max).min() * 100
        # Worst case szenario
        calmar = cagr_val / abs(max_dd) if max_dd != 0 else 0 
        # VAGR / Max DD - Effizienz unter stress
        ...
        rows.append({
            ...
            "Profit Factor":     round(pf, 2), # > 1 = Profitable Strategie 
            "Trades":            n_trades,
        })
...
def plot_monthly_heatmap(result: dict, name:str) -> None: # Hedge fund report 
    """
